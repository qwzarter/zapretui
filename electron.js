...
function createWindow() {
    ...
    if (shouldStartHidden) {
        mainWindow.hide();
        console.log('[Main Process]: Запуск из автозапуска с опцией "в трее" включен. Главное окно скрыто.');
    }
    ...
    mainWindow.on('close', async (event) => {
        if (!isQuitting && settings.get('runInTray', true)) {
            event.preventDefault();
            mainWindow.hide();
        } else {
            event.preventDefault();
...
app.whenReady().then(() => {
    ...
    ipcMain.on('update-should-start-in-tray', (event, value) => {
        ...
        console.log(`[Main Process]: Настройка 'Запускать в трее' обновлена до: ${value}`);
    });
});
